...
**최종 수정**: 2026-09-09 | **상태**: M0~M8 전부 완료 ✅ (프로덕션 배포 준비 완료)
...
| **M6 통계** | ✅ 완료 | 월별매출(LAG) + 거래처TOP(RANK) + 제품TOP + Aging(RATIO_TO_REPORT) |
| **M7 환율/알림** | ✅ 완료 | Korea Exim Bank API + 4가지 배치작업 + 실시간 NotificationPanel |
| **M8 마무리** | ✅ 완료 | README + ERD + 통계쿼리 + Swagger + Code cleanup |
...
다음 항목들은 스코프 외로 남겨둠 (프로덕션 배포에는 불필요):

- **리프레시 토큰 / 토큰 블랙리스트** — 현재는 30분 JWT + 클라이언트 폐기로 충분
  - 필요시: Redis 기반 토큰 블랙리스트 추가 (서버 재시작 후 복구 불가 보안 고려)
- **WebSocket 실시간 알림** — 현재는 배치 기반 알림 + 폴링으로 충분
  - 필요시: Spring WebSocket + STOMP로 푸시 알림 구현
- **게시글 낙관적 락** — `VERSION` 컬럼 부재 (원본 스키마 준수)
- **API Rate Limiting** — 프로토타입 단계에서는 생략
- **환율 API 다중 소스 페일오버** — 현재는 Korea Exim Bank 단일 출처
