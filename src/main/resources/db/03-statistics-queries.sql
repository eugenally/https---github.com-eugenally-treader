...
SELECT
  ...
  ROUND(((SUM(i.AMOUNT * i.EXCHANGE_RATE) /
          LAG(SUM(i.AMOUNT * i.EXCHANGE_RATE)) OVER (ORDER BY TRUNC(sh.ETD, 'MM'))) - 1) * 100, 2) AS yoy_growth_rate
FROM INVOICE i
...
FROM SALES_ORDER so
GROUP BY TRUNC(so.ORDER_DATE, 'MM')
...
SELECT
  ...
  sh.ETD,
  sh.SHIPMENT_NO,
  DATEDIFF(day, SYSDATE, sh.ETD) AS days_to_shipment
FROM SALES_ORDER so
...
SELECT
  ...
  i.DUE_DATE,
  DATEDIFF(day, SYSDATE, i.DUE_DATE) AS days_to_due
FROM INVOICE i
JOIN SALES_ORDER so ON i.ORDER_ID = so.ORDER_ID
JOIN CUSTOMER c ON so.CUSTOMER_ID = c.CUSTOMER_ID
WHERE i.STATUS IN ('ISSUED', 'PARTIALLY_PAID')
  AND DATEDIFF(day, SYSDATE, i.DUE_DATE) BETWEEN 0 AND 7
...
SELECT
  ...
  i.AMOUNT * i.EXCHANGE_RATE AS krw_amount,
  i.DUE_DATE,
  DATEDIFF(day, i.DUE_DATE, SYSDATE) AS overdue_days
FROM INVOICE i
JOIN SALES_ORDER so ON i.ORDER_ID = so.ORDER_ID
JOIN CUSTOMER c ON so.CUSTOMER_ID = c.CUSTOMER_ID
...
-- 거래처 TOP 5 바 차트
exec DBMS_OUTPUT.put_line('거래처별 매출 TOP 5:');
SELECT * FROM (
  SELECT
